...
export default {
  ...
  beforeMounte() {
    ...
    // this.$searchParams.keyword=this.$store.query.keyword;

    // Object.assign:ES6新增的语法，合并对象，简写（在给服务器发请求之前，把参数整理好，服务器就会返回查询的数据
    Obejct.assign(this.searchParams, this.$route.query, this.$route.params);
  },
  // 组件挂载完毕执行一次【仅仅执行一次
  mounted(){
    // 先测试接口返回的数据格式
  ...
  methods: {
    // 向服务器发请求获取search模块数据（更具参数不同返回不同的数据进行展示)
    ...
    removeCategiryName(){
      ...
      // 但是你把相应的字段变成undefined，当前这个字段不会带给服务器
    ...
    // 删除关键字
    removeKeyword(){
      //给服务器带的参数searchParamas的keyword置空
   ...
   removeAttr(index) {
    ...
    // 排序的操作
    changeOrder(flag){
      // flag形参：它是一个标记，代表用户点击的综合（1）价格（2）【用户点击的时候传递进来的】
      let orignOrder = this.searchParams.order;
      // console.log(orignOrder,this);
      // 这是获取到最开始的数据
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 每一次请求完毕，应该把相应的1、2、3级分类的id置空的，让他接受下一次的相应id1，2，3
